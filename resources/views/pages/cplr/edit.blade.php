...
@section('body')
    <div class="flex items-center justify-between py-5 px-5 mx-10">
        <h1 class="font-bold text-2xl mb-0">Tambah Sumber Referensi {{ $cplrEdit->kode_cplr }}</h1>
    </div>
    <hr />
    @if ($errors->any())
        <p style="color: red">{{ $errors->first() }}</p>
    @endif
    <form class="py-3" action="{{ route('update.cplr', ['id' => $cplrEdit->id]) }}" method="POST">
        @csrf
        <div class="grid md:grid-cols-2 md:gap-6">
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" name="kode_cplr" id="kode_cplr"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none   focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " value="{{ $cplrEdit->kode_cplr }}" />
                <label for="kode_cplr"
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" name="cplr" id="cplr"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none   focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " value="{{ $cplrEdit->cplr }}" />
                <label for="cplr"
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" name="deskripsi_cplr" id="deskripsi_cplr"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none   focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " value="{{ $cplrEdit->deskripsi_cplr }}" />
