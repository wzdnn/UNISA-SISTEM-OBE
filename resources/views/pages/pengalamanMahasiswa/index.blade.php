...
@section('body')
    ...
    <div class="flex items-center justify-between py-5 px-5">
        <div class="flex items-center">
            <h1 class="font-bold text-2xl mb-0 text-gray-700 text-center">Pengalaman Mahasiswa</h1>
        </div>
        <div class="flex items-center">
        ...
        <div id="defaultModal" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative w-full max-w-2xl max-h-full">
                ...
                <div class="relative bg-white px-5 py-5 rounded-lg shadow">
                    <form class="py-3" action="{{ route('post.pengalaman') }}" method="POST">
                        @csrf
                        <div class="">
                            <div class="relative z-0 w-full mb-6 group">
                                <input type="text" name="pengalaman_mahasiswa" id="pengalaman_mahasiswa"
                                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none   focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                    placeholder=" " required value="{{ old('pengalaman_mahasiswa') }}" />
                                <label for="pengalaman_mahasiswa"
                                    class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Pengalaman
                                    Sumber</label>
                            </div>
                        </div>
    ...
    <div class="relative py-3">
        <table class="w-full text-sm text-center  text-gray-500">
            ...
            <tbody>
                @if ($pengalamanMahasiswa->count() > 0)
                    @foreach ($pengalamanMahasiswa as $key => $pengalamanMahasiswas)
                        <tr class="{{ $key % 2 == 0 ? 'bg-gray-100' : 'bg-gray-50' }} border-b text-left">
                            <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                {{-- {{ $loop->iteration }} --}}
                                {{ ($pengalamanMahasiswa->currentPage() - 1) * $pengalamanMahasiswa->perPage() + $key + 1 }}
                            </td>
                            <td class="px-6 py-4">
                                {{ $pengalamanMahasiswas->pengalaman_mahasiswa }}
                            </td>
                            @if (Auth::user()->leveling !== 4)
                                <td class="px-6 py-4">

                                    {{-- <a href="{{ route('delete.pengalaman', ['id' => $pengalamanMahasiswas->id]) }}"
                                    ...
                                    <button disabled class="bg-red-600 text-white rounded px-2 text-md font-semibold p-1"><i
                                            class="fa-solid fa-trash"></i></button>
                                </td>
                            @endif
                        </tr>
                ...
                @else
                    <tr>
                        <td class="justify-center text-center" colspan="5">Pengalaman Mahasiswa belum ada</td>
                    </tr>
                @endif
            </tbody>
        </table>
        {{ $pengalamanMahasiswa->withQueryString()->links() ?? '' }}
        <hr />
    </div>
@endsection
