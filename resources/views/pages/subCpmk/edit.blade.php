...
@section('body')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Edit Sub CPMK</span>
                </div>
            </li>
    ...
    <div class="text-center">
        <h1 class="font-bold text-2xl mb-0 text-gray-700">Edit Sub CPMK</h1>
    </div>
    {{-- <hr /> --}}
    @if ($errors->any())
    ...
    <div class="my-3 mx-auto max-w-4xl">
        <div class="px-3 bg-white border border-gray-200 rounded shadow-lg justify-between">
            <form class="py-3" action="{{ route('subcpmk.update', ['id' => $sub_cpmk->kdsubcpmk]) }}" method="POST">
                @csrf
                <div class="grid md:grid-cols-2 md:gap-6">
                    <div class="relative z-0 w-full mb-6 group">
                        <input type="text" name="kode_subcpmk" id="kode_subcpmk"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none   focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder=" " required value="{{ $sub_cpmk->kode_subcpmk }}" />
                        <label for="kode_subcpmk"
                    ...
                    <div class="relative z-0 w-full mb-6 group">
                        <input type="text" name="sub_cpmk" id="sub_cpmk"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none   focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder=" " required value="{{ $sub_cpmk->sub_cpmk }}" />
                        <label for="sub_cpmk"
                    ...
                    <div class="flex flex-col z-0 w-full mb-6 group">
                        ...
                        <select id="cpmk" name="cpmk" class="form-control">
                            @foreach ($cpmk as $cpmks)
                                <option value="{{ $cpmks->id }}">{{ $cpmks->kode_cpmk }} {{ $cpmks->cpmk }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="flex flex-col z-0 w-full mb-6 group">
