        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Kelola Sub BK</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <form action="" method="post">
        ...
        <select class="w-full" id="subbk" name="subbk[]" multiple="multiple">
            @foreach ($subbk as $item)
                <option value="{{ $item->id }}" @selected(in_array($item->id, $subbkSelect))>{{ $item->kode_subbk }} {{ $item->sub_bk }}
                </option>
            @endforeach
        </select>
